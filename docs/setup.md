...
Multi-edge is on by default. Acquisition requires `AUTOMATION_NODE_ID` and `AUTOMATION_AREA`; API/HMI remain available if they are missing. See [multi-edge.md](multi-edge.md).
