...
Multi-edge is on by default. Acquisition requires `AUTOMATION_NODE_ID` and `AUTOMATION_SEGMENT` (or alias `AUTOMATION_AREA`); `AUTOMATION_MANUFACTURER` is the site alias. API/HMI remain available if they are missing. See [multi-edge.md](multi-edge.md).
